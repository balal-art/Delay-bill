...
def dilly_out(dataframe):
    frames = [movaghat(dataframe, date0= dataA.date0),movaghat(dataframe, type= mystr.tadil, date0=dataA.date0)]
...
result = dilly_out(data)



# df = pd.DataFrame().assign(
#                 Task=result[mystr.type]+ ' ' + result[mystr.num],
#                 Start=pd.to_datetime(result[mystr.date_tamdid]),
#                 Finish=pd.to_datetime(result[mystr.date_pardakht])
#             )

start_date = pd.to_datetime(result[mystr.date_tamdid]).min()
endTask = pd.to_datetime(result[mystr.date_pardakht])
startTask = pd.to_datetime(result[mystr.date_tamdid])



task = df2fa(result[mystr.type]+ ' ' + result[mystr.num])
end = (endTask - start_date).dt.days
start = (startTask - start_date).dt.days

# print(df2fa(y))
# fig, ax = plt.subplots(1, figsize=(16,6))
  
# creating the bar plot
# plt.barh(task, end, left=start, color='maroon')

# ax = ax.xaxis_date()
# ax = plt.barh(task, dt.date2num(endTask), left=dt.date2num(startTask), color='maroon')
...
endTask, startTask = [mdates.date2num(item) for item in (endTask, startTask)]

ypos = range(len(endTask))
fig, ax = plt.subplots()

# Plot the data
ax.barh(task, endTask - startTask, left=startTask, height=0.8, align='center')
ax.axis('tight')
...
# ax.xlabel(_("مدت به روز"))
# ax.ylabel(_("صورتحساب"))
# ax.title(_("نمودار همپوشانی براساس بخشنامه 5090"))
